        ...
        for col in range(2):
            ...
            if idx < len(PAGE_CONFIG):
                with cols[col]:
                    ...
                    st.image(page["thumbnail"], use_column_width=True, caption=page["description"])
                    with st.expander("游戏介绍"):
...
def show_subpage():
    """显示游戏页面"""
    st.components.v1.iframe(st.session_state.current_page, height=600)
    if st.button("返回主界面"):
...
if __name__ == "__main__":
    ...
    # 用户已登录显示主界面
        ...
        with st.container():
            ...
            with cols[1]:
                if st.button("💬 AI对话"):
                    st.switch_page("pages/1_💬_文生文应用.py")
            with cols[2]:
                if st.button("🚪 退出"):
